...
for (let e = 16; e > 0; e--) {
  t += (1e8 * Math.random() | 0).toString(36)[0];
}
...
function encodeName(e) {
  return e.replace(n, pencode);
}

const i = /\\(?:u[0-9a-f]{4}|x[0-9a-f]{2}|.)/gi;
...
function decodeName(e) {
  return e.replace(i, decodeBackslashEscape).replace(a, pdecode);
}

const r = new Uint8Array([ 13, 10 ]);

const o = bytesToSkipTable(r);
...
function utf8Encode(e) {
  return "string" == typeof e ? l.encode(e) : new Uint8Array("buffer" in e ? e.buffer : e);
}
...
async function decodeHeaders(e) {
  ...
  while (t < 32e3) {
    let a = "";
    for await (const i of readUntilBoundary(e, r, o)) {
      if (null == i) {
        throw new Error("Invalid Multipart Headers: Unexpected EOF");
      } else if (!t && !a && 45 === i[0] && 45 === i[1]) {
        return null;
      } else {
        a += n.decode(i);
        if (a.length > 16e3) {
    ...
    if (a) {
      const e = a.indexOf(":");
      if (e > -1) {
        const n = a.slice(0, e).trim().toLowerCase();
        const o = a.slice(e + 1).trim();
        if (o) {
          i[n] = o;
        }
        t += a.length + r.byteLength;
      } else {
...
const makeFormHeader = (e, n) => {
  let i = d + t + c;
  i += `Content-Disposition: form-data; name="${encodeName(e.name)}"`;
  if (null != e.filename) {
    i += `; filename="${encodeName(e.filename)}"`;
  }
  if (n) {
    if (n.type) {
      i += `${c}Content-Type: ${n.type}`;
    }
    if (n.size) {
      i += `${c}Content-Length: ${n.size}`;
    }
    if ("headers" in n) {
...
const f = `multipart/form-data; boundary=${t}`;
...
function decodeString(e, t, n) {
  ...
  return i > t ? m.decode(e.subarray(t, i)) : "";
}
...
function encodeString(e, t, n, i) {
  if (i) {
    y.encodeInto(`${i}\0`, e.subarray(t, n));
  }
}
...
function encodeHeader(e) {
  ...
  !function encodeBase(e, t) {
    let n = t.name;
    if (!t._paxName && !t._longName && n.length > p) {
    ...
    if (!t.mode) {
      t.mode = t.typeflag === h.DIRECTORY ? 493 : 420;
    }
    if (!t.mtime) {
...
async function* tar(e) {
  ...
  while (!(n = await t()).done && n.value) {
    ...
    if (t.typeflag === h.DIRECTORY && !t.name.endsWith("/")) {
      t.name += "/";
    } else if (t.typeflag === h.SYMLINK) {
      t.size = 0;
    }
    if (t.name.length > p) {
    ...
    if (t.linkname && t.linkname.length > p) {
...
export { MultipartPart, StreamFile, TarChunk, TarFile, h as TarTypeFlag, iterableToStream, f as multipartContentType, parseMultipart, streamMultipart, streamToAsyncIterable, tar, untar };
